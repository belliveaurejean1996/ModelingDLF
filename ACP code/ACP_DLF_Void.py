        ...
        for j in range(PLY+1):
            ...
            if Void >= VoidFill:
                ...
                for k in range(len(VoidIndex)): 
                    Orientation[VoidIndex[k], j] = 90.1
...
for i in range(stackup.shape[1]):
    ...
    #if i < (stackup.shape[1]-1):
    #    myGroup.create_interface_layer()
    #    myModel.selection.set([myModel.modeling_groups['Layers'].plies['InterfaceLayer.'+str(i+1)]])
    #    myModel.selection.set([])
    #    myModel.selection.set([myModel.oriented_selection_sets['OrientedSelectionSet.1']])
    #    myModel.modeling_groups['Layers'].plies['InterfaceLayer.'+str(i+1)].oriented_selection_sets = (myModel.oriented_selection_sets['OrientedSelectionSet.1'])
    #    myModel.selection.set([myModel.modeling_groups['Layers'].plies['InterfaceLayer.'+str(i+1)]])
