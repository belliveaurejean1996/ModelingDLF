        ...
        for j in range(PLY+1):
            ...
            if Void >= VoidFill:
                ...
                for k in range(len(VoidIndex)): 
                    Orientation[VoidIndex[k], j] = 90.05
...
# generate the files for ansys APDL
num = 1
for k in range(25):
    # create the stackups
    ...
    reshapedStackup = np.reshape(stackup.T, Q*PLY)

    # create the tables to import to Ansys
    values, count = np.unique(reshapedStackup, return_index=True)

    NumCoordinate = np.zeros((len(values),2))
    ElmCoordinate = np.zeros((len(reshapedStackup),2))

    for i in range(len(values)):
        NumCoordinate[i,1] = values[i]
        NumCoordinate[i,0] = i + 11

    for i in range(len(reshapedStackup)):
        for j in range(len(values)):
            if reshapedStackup[i] == values[j]:
                ElementValue = NumCoordinate[j,0]

        ElmCoordinate[i,0] = i+1
        ElmCoordinate[i,1] = ElementValue

    # export the values
    np.savetxt(r'E:\Universite\Matrise\Article - Comparison\AnsysWorking\Stiffness\Data' + str(num) + '.csv', NumCoordinate, delimiter=",")
    np.savetxt(r'E:\Universite\Matrise\Article - Comparison\AnsysWorking\Stiffness\DataElm' + str(num) + '.csv', ElmCoordinate, delimiter=",")
